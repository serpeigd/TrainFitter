# TrainFitter — Full Status History

> Chronological development log, session by session -- every feature, bug
> fix, and direct request behind it, with how each was verified. Moved out
> of `CLAUDE.md`'s own "Status" section (2026-09-07) once that section grew
> past 100KB and kept growing every session -- same "full log vs. condensed
> pointer" pattern already used for `docs/decisiones.md` vs.
> `docs/highlights.md`. Read this only if you need the *history* behind a
> specific past decision, bug, or test-count number; the condensed
> current-state summary lives in `CLAUDE.md`'s own "Status" section instead,
